...
export default function FloodMap({ lat = -20.2, lng = 57.5 }: FloodMapProps) {
  ...
  const updateLayers = (map: MapboxMap) => {
    ...
    if (layers.flood.rainfall24h && prediction.rainfall.hourlyPrecip24h) {
      const points = generateRainfallHeatmapPoints(
        prediction.rainfall.hourlyPrecip24h,
        lat,
        lng,
        0.2
      );
    ...
    if (layers.flood.rainfall72h && prediction.rainfall.hourlyPrecip72h) {
      const points = generateRainfallHeatmapPoints(
        prediction.rainfall.hourlyPrecip72h,
        lat,
        lng,
        0.25
      );
    ...
    const popupHTML = `
      <div style="width:320px;font-family:'Figtree',sans-serif;background:#fff;border-radius:10px;box-shadow:0 4px 16px rgba(0,0,0,0.25);overflow:hidden;padding:16px;">
        ...
        <div style="space-y:8px;font-size:14px;">
          ...
          <div>
            <span style="font-weight:600;">Current Intensity:</span>{' '}
            ${(prediction.rainfall.currentIntensity || 0).toFixed(1)} mm/h
          </div>
          <div>
