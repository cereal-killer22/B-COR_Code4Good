  ...
  async function fetchOceanHealth() {
    try {
      ...
      if (response.ok) {
        ...
        // Transform API response to OceanHealthMetrics format with default values
        if (data.metrics) {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <div className="flex items-center justify-between mb-6">
          ...
          <StatusBadge status={(healthData.overallHealthScore ?? 75) >= 70 ? 'success' : (healthData.overallHealthScore ?? 75) >= 50 ? 'warning' : 'danger'}>
            {(healthData.overallHealthScore ?? 75) >= 80 ? 'Excellent' : 
             (healthData.overallHealthScore ?? 75) >= 60 ? 'Good' : 
             (healthData.overallHealthScore ?? 75) >= 40 ? 'Moderate' : 'Poor'}
          </StatusBadge>
        </div>
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className={`text-4xl font-bold ${getHealthColor(healthData.overallHealthScore ?? 75)}`}>
              {healthData.overallHealthScore ?? 75}/100
            </span>
          </div>
          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-8 overflow-hidden">
            <div 
              className={`h-full rounded-full transition-all ${getHealthBgColor(healthData.overallHealthScore ?? 75)}`}
              style={{ width: `${healthData.overallHealthScore ?? 75}%` }}
            />
          </div>
      ...
      <Card>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          <MetricCard
            ...
            trend={{
              direction: (healthData.waterQuality?.pH ?? 8.1) >= 8.0 ? 'up' : 'down',
              value: getPHStatus(healthData.waterQuality?.pH ?? 8.1).status
            }}
          />
          <MetricCard
            icon="🌡️"
        ...
        <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-2xl font-bold ${getHealthColor(healthData.waterQuality?.score ?? 75)}`}>
              {healthData.waterQuality?.score ?? 75}/100
            </span>
          </div>
          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3 mt-2 overflow-hidden">
            <div 
              className={`h-full rounded-full ${getHealthBgColor(healthData.waterQuality?.score ?? 75)}`}
              style={{ width: `${healthData.waterQuality?.score ?? 75}%` }}
            />
          </div>
        </div>
      </Card>
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <MetricCard
            ...
            value={`${(healthData.pollution?.plasticDensity ?? 15).toFixed(1)}`}
            subtitle="particles/km²"
            trend={{
              direction: (healthData.pollution?.plasticDensity ?? 15) < 20 ? 'down' : 'up',
              value: (healthData.pollution?.plasticDensity ?? 15) < 20 ? 'Low' : 'High'
            }}
          />
          <MetricCard
            icon="🛢️"
            title="Oil Spill Risk"
            value={`${healthData.pollution?.oilSpillRisk ?? 20}%`}
            subtitle="Risk level"
          />
          <MetricCard
            icon="🧪"
            title="Chemical Pollution"
            value={`${healthData.pollution?.chemicalPollution ?? 25}%`}
            subtitle="Contamination risk"
          />
        </div>
        <div className="p-4 rounded-lg border border-gray-200 dark:border-gray-700" style={{
          background: (healthData.pollution?.overallIndex ?? 20) < 30 
            ? 'linear-gradient(to right, #10b981, #059669)' 
            : (healthData.pollution?.overallIndex ?? 20) < 50
          ...
          <div className="flex items-center justify-between text-white">
            ...
            <span className="text-2xl font-bold">
              {healthData.pollution?.overallIndex ?? 20}/100
            </span>
          </div>
          <div className="text-sm text-white/90 mt-1">
            {(healthData.pollution?.overallIndex ?? 20) < 30 ? '✅ Low pollution - Excellent water quality' :
             (healthData.pollution?.overallIndex ?? 20) < 50 ? '⚠️ Moderate pollution - Monitoring recommended' :
      ...
      <Card>
        ...
        <div className="bg-gradient-to-r from-red-50 via-yellow-50 to-green-50 dark:from-red-900/20 dark:via-yellow-900/20 dark:to-green-900/20 p-6 rounded-lg border border-gray-200 dark:border-gray-700 mb-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <StatusBadge status={getRiskBadgeColor(healthData.reefHealth?.bleachingRisk ?? 'low')}>
              {(healthData.reefHealth?.bleachingRisk ?? 'low').toUpperCase()}
            </StatusBadge>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div>
              <div className="text-sm text-theme-secondary mb-1">Health Index</div>
              <div className={`text-2xl font-bold ${getHealthColor(healthData.reefHealth?.healthIndex ?? 75)}`}>
                {healthData.reefHealth?.healthIndex ?? 75}/100
              </div>
            </div>
            <div>
              <div className="text-sm text-theme-secondary mb-1">Temperature</div>
              <div className="text-2xl font-bold text-theme">{(healthData.reefHealth?.temperature ?? 28.5).toFixed(1)}°C</div>
            </div>
            <div>
              <div className="text-sm text-theme-secondary mb-1">pH</div>
              <div className="text-2xl font-bold text-theme">{(healthData.reefHealth?.pH ?? 8.1).toFixed(2)}</div>
            </div>
            <div>
              <div className="text-sm text-theme-secondary mb-1">Coral Coverage</div>
              <div className="text-2xl font-bold text-theme">{(healthData.reefHealth?.coverage ?? 40).toFixed(0)}%</div>
            </div>
            <div>
              <div className="text-sm text-theme-secondary mb-1">Status</div>
              <div className={`text-lg font-bold ${getHealthColor(healthData.reefHealth?.healthIndex ?? 75)}`}>
                {(healthData.reefHealth?.healthIndex ?? 75) >= 80 ? 'Healthy' :
                 (healthData.reefHealth?.healthIndex ?? 75) >= 60 ? 'Stable' :
                 (healthData.reefHealth?.healthIndex ?? 75) >= 40 ? 'Stressed' : 'Critical'}
              </div>
            </div>
          </div>
        </div>
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <MetricCard
            icon="🐠"
            title="Species Count"
            value={healthData.biodiversity?.speciesCount ?? 200}
            subtitle="Total species"
            trend={{
              direction: (healthData.biodiversity?.speciesCount ?? 200) > 200 ? 'up' : 'down',
              value: (healthData.biodiversity?.speciesCount ?? 200) > 200 ? 'High' : 'Moderate'
            }}
          />
          <MetricCard
            icon="⚠️"
            title="Endangered Species"
            value={healthData.biodiversity?.endangeredSpecies ?? 5}
            subtitle="At risk"
            trend={{
              direction: (healthData.biodiversity?.endangeredSpecies ?? 5) < 5 ? 'down' : 'up',
              value: (healthData.biodiversity?.endangeredSpecies ?? 5) < 5 ? 'Low' : 'High'
            }}
          />
          <MetricCard
            icon="📊"
            title="Biodiversity Index"
            value={`${healthData.biodiversity?.biodiversityIndex ?? 70}/100`}
            subtitle="Overall diversity"
            trend={{
              direction: (healthData.biodiversity?.biodiversityIndex ?? 70) >= 70 ? 'up' : 'down',
              value: (healthData.biodiversity?.biodiversityIndex ?? 70) >= 70 ? 'High' : 'Moderate'
            }}
          />
        </div>
          ...
          <div className="text-sm text-theme-secondary mb-2">
            Biodiversity assessment based on water clarity, primary productivity, and habitat health
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-lg font-bold ${getHealthColor(healthData.biodiversity?.biodiversityIndex ?? 70)}`}>
                    {(healthData.biodiversity?.biodiversityIndex ?? 70) >= 70 ? 'High Diversity' :
                     (healthData.biodiversity?.biodiversityIndex ?? 70) >= 50 ? 'Moderate Diversity' : 'Low Diversity'}
                  </span>
                </div>
        </div>
      </Card>
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1">
          <div className="relative h-[650px] min-h-[650px] md:h-[75vh] rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700">
            <OceanHealthMap lat={location[0]} lng={location[1]} />
          </div>
        </div>
